...
static void make_csr(const packed_edge* restrict const inbuf, temp_csr_graph* restrict const outg /* Must have memory and nlocalverts/nlocaledges filled in */) {
  size_t nrows = (size_t) outg->nlocalverts;
  size_t inbuf_size = (size_t) outg->nlocaledges;
  size_t* temp = (size_t*)xmalloc(nrows * sizeof(size_t));
  size_t* restrict rowstarts = (size_t*) outg->rowstarts;
  int64_t* restrict column = (int64_t *) outg->column;
  {
...
#pragma omp parallel for
    for (i = 0; i < (ptrdiff_t)inbuf_size; ++i) {
      // extra debugging
      size_t ival = (VERTEX_LOCAL(get_v0_from_edge(&inbuf[i])));
      if (ival >= nrows) {
        printf("make_csr: ival %ld nrows %ld i %ld get_v0 %ld\n",
              (long) ival, (long) nrows, (long) i, (long) get_v0_from_edge(&inbuf[i]) );
        printf("outg:nlocalverts %ld nlocaledges %ld rowstarts %ld column %ld \n", 
                (long) outg->nlocalverts, (long) outg->nlocaledges, 
                (long) outg->rowstarts, (long) outg->column );
        };
...
#pragma omp atomic
    ...
    rowstarts[0] = 0;
    for (i = 0; (size_t) i < nrows; ++i) {
      rowstarts[i + 1] = rowstarts[i] + counts[i];
    }
  }
  {
    size_t* restrict inserts = temp;
    memcpy(inserts, rowstarts, ((size_t) nrows) * sizeof(size_t));
    ptrdiff_t i;
#pragma omp parallel for
    for (i = 0; i < (ptrdiff_t)inbuf_size; ++i) {
...
#pragma omp parallel for
    for (i = old_b_nlocalverts; ((size_t) i) < b_nlocalverts; ++i) {
...
#define CONV1D_BUILD_FINAL_DATA_STRUCTURE_FROM_GRAPH_SO_FAR \
  ...
  g->max_nlocalverts = shmem_long_max_all( (long) nlocalverts );  \
  /* MPI_Allreduce(&nlocalverts, &g->max_nlocalverts, 1, MPI_INT64_T, MPI_MAX, MPI_COMM_WORLD); */\
  g->max_nlocalverts = shmem_long_max_all( nlocalverts );  \
...
#endif
#endif
void print_oned_csr_graph(const char *path, oned_csr_graph *g)
{
  int rank = 0;
  int size = 1;
  ...
  FILE *fp = NULL;
  int istat = 0;
  int64_t ei,ei_start,ei_end;
  int64_t vlocal,vglobal,wglobal;
  int64_t nlocalverts = (*g).nlocalverts;
  size_t *rowstarts = (*g).rowstarts;
  ...
  for(pe = 0; pe < size; pe++) {
   ...
   if (pe == rank) {
    ...
    assert( istat != -1 );
    fprintf(fp,"%% rank %d nlocalverts %ld\n",rank,nlocalverts);
    for(vlocal=0; vlocal < nlocalverts; vlocal++) {
      ei_start = rowstarts[vlocal];
      ei_end = rowstarts[vlocal+1];
      vglobal = VERTEX_TO_GLOBAL(rank,vlocal);

      for( ei=ei_start; ei < ei_end; ei++) {
         wglobal = column[ei];
         fprintf(fp,"%ld %ld\n",(long) vglobal,(long) wglobal);
         };
      };
     istat = fclose(fp);
     assert( istat == 0);
