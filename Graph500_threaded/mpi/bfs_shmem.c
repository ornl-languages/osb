...
void run_bfs(int64_t root, int64_t* pred) {
  ...
  {unsigned long i; for (i = 0; i < nlocalverts; ++i) local_vertices[i] = VERTEX_TO_GLOBAL(rank, i);}
...
#ifdef USE_OPENSHMEM
  assert( shmem_addr_accessible_all( pred ) );
  assert( shmem_addr_accessible_all( pred2 ) );
  assert( shmem_addr_accessible_all( queue_bitmap1 ) );
  assert( shmem_addr_accessible_all( queue_bitmap2 ) );
  shmem_barrier_all();
#else
  ...
  while (1) {
    ...
    memset(queue_bitmap2, 0, ((size_t) queue_nwords) * sizeof(unsigned long));
...
#pragma omp parallel
{
    ...
    for (i = 0; i < queue_nwords; ++i) {
      ...
      for (bitnum = 0; bitnum < ulong_bits; ++bitnum) {
        ...
        if (first_v_local >= nlocalverts) { 
            /* faking a break due to openmp */
            bitnum = ulong_bits;
            continue;
        }
        ...
        for (qelem_idx = 0; qelem_idx < elts_per_queue_bit; ++qelem_idx) {
          ...
          if (pred[v_local] >= 0 && pred[v_local] < nglobalverts) {
            ...
            for (ei = g.rowstarts[v_local]; ei < ei_end; ++ei) {
              ...
              if (w == ((int64_t) VERTEX_TO_GLOBAL(rank, v_local))) continue; /* Self-loop */
              /* Set the predecessor of the other edge endpoint (note use of
               * MPI_MIN and the coding of the predecessor map). */
#ifdef USE_OPENSHMEM
             {
                int pe = VERTEX_OWNER(w);
                long offset = (long) VERTEX_LOCAL(w);
                long lvalue = local_vertices[v_local];
                long *ptarget = &(pred2[offset]);

#ifndef WITH_CTX
                shmem_long_min(ptarget,lvalue,pe);    
#else
                shmem_ctx_long_min( ptarget, lvalue, pe, ct);
#endif
              }

#else
              MPI_Accumulate(&local_vertices[v_local], 1, MPI_INT64_T,
                             VERTEX_OWNER(w), VERTEX_LOCAL(w), 1, MPI_INT64_T, MPI_MIN,
                             pred2_win);
#endif
             ...
             {
                int pe = VERTEX_OWNER(w);
                size_t offset =  VERTEX_LOCAL(w) / elts_per_queue_bit / ulong_bits; 
                unsigned long ulvalue = masks[((VERTEX_LOCAL(w) / elts_per_queue_bit) % ulong_bits)]; 
                unsigned long *ptarget =  &(queue_bitmap2[offset]);

#ifndef WITH_CTX
                shmem_ulong_bor( ptarget, ulvalue, pe );
#else
                shmem_ctx_ulong_bor(ptarget, ulvalue, pe, ct);
#endif
             };
#else
              MPI_Accumulate(&masks[((VERTEX_LOCAL(w) / elts_per_queue_bit) % ulong_bits)], 
                      1, MPI_UNSIGNED_LONG,
                      VERTEX_OWNER(w), 
...
#ifdef USE_OPENSHMEM
    {
        any_set = shmem_int_lor_all(  any_set );
    }
#else
  ...
  xMPI_Free_mem(local_vertices);
  xMPI_Free_mem(queue_bitmap1);
  xMPI_Free_mem(queue_bitmap2);
...
int64_t vertex_to_global_for_pred(int v_rank, size_t v_local) {
  return (int64_t) VERTEX_TO_GLOBAL(v_rank, v_local);
}

size_t get_nlocalverts_for_pred(void) {
  return (size_t) g.nlocalverts;
}
