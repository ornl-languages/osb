...
#pragma omp critical
  ...
  if (use_mpi_lock) {
    ierr = MPI_Win_lock( lock_type, remote_rank, iassert, g->win);
...
#ifdef  USE_MPI_WIN_LOCK
  if (use_mpi_lock) {
      ierr = MPI_Win_unlock( remote_rank, g->win );
      assert(ierr == MPI_SUCCESS );
     };
  };
#endif
...
#pragma omp critical
  ...
  if (use_mpi_lock) {
    ierr = MPI_Win_lock( lock_type, remote_rank, iassert, sc->win);
    assert( ierr == MPI_SUCCESS );
    };
#endif
...
#ifdef  USE_MPI_WIN_LOCK
  if (use_mpi_lock) {
      ierr = MPI_Win_unlock( remote_rank, sc->win );
      assert(ierr == MPI_SUCCESS );
     };
  };
#endif
