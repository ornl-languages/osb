...
int main(int argc, char** argv) {
#ifdef USE_OPENSHMEM
    /* TJN: Not sure what CRAY_SHMEM flag we can check so add quick fix */
 #ifdef X_HAVE_CRAY_SHMEM  
    ...
    int i = 0;
    ctx = (shmem_ctx_h *) malloc(sizeof(shmem_ctx_h) * nr_threads);
    for (i = 0; i < nr_threads; i++) {
        shmem_ctx_create(&ctx[i]);
    }
#endif
#else
  MPI_Init(&argc, &argv);
