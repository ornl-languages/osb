...
static score_matrix_t *alloc_score_matrix(index_t matrix_length){
  ...
  new_alloc->scores=shmem_malloc(sizeof(score_t)*3*new_alloc->local_length);
...
static gap_matrix_t *alloc_gap_matrix(index_t matrix_length){
  ...
  new_alloc->scores = shmem_malloc(sizeof(score_t)*2*new_alloc->local_length);
  assert(new_alloc->scores != NULL);
...
static void fetch_score(score_matrix_t *A, index_t m, index_t n, score_t *in){
  ...
  shmem_short_get(in, &(A->scores[index2d(m%3,local_index,A->local_length)]), 1, target_ep);
}

static void fetch_gap(gap_matrix_t *A, index_t m, index_t n, score_t *in){
  int target_ep = n / A->local_length;
  int local_index = n %A->local_length;
  shmem_short_get(in, &(A->scores[index2d(m%2,local_index,A->local_length)]), 1, target_ep);
}
...
static void assign_score(score_matrix_t *A, index_t m, index_t n, score_t new_value){
  ...
  shmem_short_put(&(A->scores[index2d(m%3,local_index,A->local_length)]), &new_value, 1, target_ep);
}
...
static void assign_gap(score_matrix_t *A, index_t m, index_t n, score_t new_value){
  ...
  shmem_short_put(&(A->scores[index2d(m%2,local_index,A->local_length)]), &new_value, 1, target_ep);
}
...
good_match_t *pairwise_align(seq_data_t *seq_data, sim_matrix_t *sim_matrix, const int minScore, const int maxReports, const int minSeparation) {
  ...
  for(int idx=0; idx < max_threads; idx++) {
    good_ends[idx] = shmem_malloc(sizeof(current_ends_t));
    good_ends[idx]->size = sortReports;
    good_ends[idx]->report = 0;
    good_ends[idx]->goodScores = shmem_malloc(sizeof(score_t)*sortReports);
    good_ends[idx]->goodEnds[0] = shmem_malloc(sizeof(index_t)*sortReports);
    good_ends[idx]->goodEnds[1] = shmem_malloc(sizeof(index_t)*sortReports);
...
#pragma omp parallel for \
    firstprivate(next_main, next_match, next_G, next_E, next_F) \
  ...
  for(int idx=0; idx < max_threads; idx++) {
    shmem_free(good_ends[idx]->goodScores);
    shmem_free(good_ends[idx]->goodEnds[0]);
    shmem_free(good_ends[idx]->goodEnds[1]);
    shmem_free(good_ends[idx]);
  }
