...
{     \
  size_t size_in_bytes = sizeof( *(edges_to_send) ); \
  shmem_mem_alltoallv( edges_to_send,edge_counts_per_owner,edge_displs_per_owner, \
                       edges_to_recv,edge_counts_per_sender,edge_displs_per_sender2, \
                       size_in_bytes ); \
}
...
#define ALLTOALLV_EDGE( edges_to_send,edge_counts_per_owner,edge_displs_per_owner, rf_edge_buffer_mpi_type1, edges_to_recv, edge_counts_per_sender, edge_displs_per_sender, rf_edge_buffer_mpi_type2, comm ) \
      { MPI_Alltoallv(edges_to_send, edge_counts_per_owner, edge_displs_per_owner, rf_edge_buffer_mpi_type1, \
                    edges_to_recv, edge_counts_per_sender, edge_displs_per_sender, rf_edge_buffer_mpi_type2, \
                    comm); } 
#endif
...
void RF_FUNCNAME(const tuple_graph* const tg, RF_EXTRA_PARAMS) { \
  ...
  int* edge_counts_per_owner = (int*)xmalloc(((size_t) size) * sizeof(int)); \
  int* edge_counts_per_sender = (int*)xmalloc(((size_t) size) * sizeof(int)); \
  int* edge_displs_per_owner = (int*)xmalloc((((size_t) size) + 1) * sizeof(int)); \
  int* edge_displs_per_sender = (int*)xmalloc(( ((size_t) size) + 1) * sizeof(int)); \
  int* edge_inserts_per_owner = (int*)xmalloc( ((size_t) size) * sizeof(int)); \
  ...
  do { \
    ...
    ITERATE_TUPLE_GRAPH_BEGIN(tg, buf, bufsize) { \
      ...
      ALLTOALL_INT( edge_counts_per_owner, edge_counts_per_sender ); \
   ...
   \
   ALLTOALLV_EDGE( edges_to_send,edge_counts_per_owner,edge_displs_per_owner, RF_EDGE_BUFFER_MPI_TYPE, edges_to_recv, edge_counts_per_sender, edge_displs_per_sender, RF_EDGE_BUFFER_MPI_TYPE, MPI_COMM_WORLD ); \
      xMPI_Free_mem(edges_to_send); \
   \
      { \
        RF_PRECOMPRESS_INCOMING_DATA(lg_nglobalverts, edges_to_recv, edges_received_this_block) \
        xMPI_Free_mem(edges_to_recv); \
