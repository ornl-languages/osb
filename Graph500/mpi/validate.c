...
#ifdef USE_OPENSHMEM
  {
  int my_range_errors = any_range_errors;

  any_range_errors = shmem_int_lor_all( any_range_errors );

  if (my_range_errors) {
     assert( any_range_errors );
     };
...
#ifdef USE_OPENSHMEM
  gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int64_t), pred_pred, size_min(CHUNKSIZE, nlocalverts), size_min(CHUNKSIZE, nlocalverts), 0);
#else
...
#ifdef USE_OPENSHMEM
    {
    int my_changes = any_changes;
    any_changes = shmem_int_lor_all( any_changes );
    if (my_changes) {
          assert( any_changes );
          };
    }
...
int validate_bfs_result(const tuple_graph* const tg, const int64_t nglobalverts, const size_t nlocalverts, const int64_t root, int64_t* const pred, int64_t* const edge_visit_count_ptr) {
  ...
  *edge_visit_count_ptr = 0; /* Ensure it is a valid pointer */
...
#if defined(USE_DEBUG) && defined(USE_OPENSHEM)
    fprintf(stderr, "%d: nlocalverts = %ld nglobalverts = %ld pred = %ld \n", 
		 shmem_my_pe(),(long) nlocalverts, (long) nglobalverts, (long) pred );
#endif
    ranges_ok = 0;
  }
  ...
  /* 
   * --------------------------------------------------
   * number of vertices should fit in long integer type
   * --------------------------------------------------
   */
  long local_val = (long) maxlocalverts_ui;
  unsigned long ulocal_val = (unsigned long) local_val;

  assert( ulocal_val == maxlocalverts_ui );

  maxlocalverts_ui = (unsigned long) shmem_long_max_all( local_val );
...
#pragma omp parallel for reduction(&&:validation_passed)
...

  {
...
#ifdef USE_OPENSHMEM
    gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int64_t), edge_preds, 2 * edge_chunk_size, 2 * edge_chunk_size, 0);
#else
    ...
    unsigned char one = 1;
#ifdef USE_OPENSHMEM
    scatter_constant* pred_valid_win = init_scatter_constant((void*)pred_valid, nlocalverts, sizeof(unsigned char), &one, 2 * edge_chunk_size, 0);
#else
...
#ifdef USE_DEBUG
            fprintf(stderr,"%d: line 396: i %ld i_start %ld i_end %ld\n",
                    rank,(long) i, (long) i_start, (long) i_end );
            fprintf(stderr,"%d: line 396: src %ld src_depth %d tgt %ld tgt_depth %d\n",
                rank, (long) src, (int) src_depth, (long) tgt, (int) tgt_depth );
            {
            const packed_edge *p = &buf[i];
            assert( p != NULL );
#ifdef GENERATOR_USE_PACKED_EDGE_TYPE
            int64_t v0 = p->v0_low;
            int64_t v1 = p->v1_low;
            uint32_t high = p->high;
            uint32_t high2 = ((v0 >> 32) & 0xFFFF) | (((v1 >> 32) & 0xFFFF) << 16);
            int is_same = (high == high2);
            fprintf(stderr,"%d: i %ld v0_low %ld v1_low %ld high %ld is_same %d \n",
                rank, (long) i, (long) v0, (long) v1, (long) high, is_same );
#else
            int64_t v0 = p->v0;
            int64_t v1 = p->v1;
            fprintf(stderr,"%d: i %ld v0 %ld v1 %ld \n",
                            rank, (long) i, (long) v0, (long) v1 );
...
#ifdef USE_DEBUG
            fprintf(stderr,"%d: line 402: i %ld i_start %ld i_end %ld\n",
                   rank,(long) i, (long) i_start, (long) i_end );
            fprintf(stderr,"%d: line 402: src %ld src_depth %d tgt %ld tgt_depth %d\n",
                rank, (long) src, (int) src_depth, (long) tgt, (int) tgt_depth );
...
#pragma omp parallel for reduction(&&:validation_passed)
...
#ifndef NDEBUG
       fprintf(stderr,"i %ld nlocalverts %ld p = %ld \n", 
                 (long) i, (long) nlocalverts, (long) p );
       fprintf(stderr,"root_owner %ld root_local %ld \n", 
                (long) root_owner, (long) root_local );
       fprintf(stderr,"pred_valid[i] %ld \n", (long) pred_valid[i] );

#endif
        validation_passed = 0;
      }
    }
    xMPI_Free_mem(pred_valid);

#ifdef USE_OPENSHMEM
    {
    long lvalue = edge_visit_count;
    assert( edge_visit_count >= 0);

#ifdef USE_DEBUG
    if (edge_visit_count > 0) {
      printf("before shmem_long_sum_all: rank %d edge_visit_count %ld\n", 
              rank,edge_visit_count);
    };
#endif
    ...
    edge_visit_count = gvalue;

#ifdef USE_DEBUG
    if (rank == 0) {
      printf("after shmem_long_sum_all, edge_visit_count %ld\n", edge_visit_count);
    };
#endif
...
#ifdef USE_DEBUG
    if (edge_visit_count > 0) {
      printf("before MPI_Allreduce:  rank %d edge_visit_count %ld \n",
              rank,edge_visit_count);
    };
#endif
...
#ifdef USE_DEBUG
    if (rank == 0) {
      printf("after MPI_Allreduce:  edge_visit_count %ld \n",edge_visit_count);
    };   
#endif
...
#ifdef USE_OPENSHMEM
  {
  int my_validation_passed = validation_passed;
  validation_passed = shmem_int_land_all( validation_passed );
  if (!my_validation_passed) { 
