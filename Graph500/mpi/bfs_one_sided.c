...
#ifdef USE_DEBUG
  print_oned_csr_graph("mpi_graph.txt",&g);
#endif  
}
          ...
          if (pred[v_local] >= 0 && pred[v_local] < nglobalverts) {
            ...
            for (ei = g.rowstarts[v_local]; ei < ei_end; ++ei) {
...
#ifdef USE_MPI_WIN_LOCK
              {
               ...
               int ierr = 0;
               int iassert = 0;
               int irank = VERTEX_OWNER(w);
               int lock_type = MPI_LOCK_SHARED;
               const int use_mpi_lock = 1;


               if (use_mpi_lock) {
                 ierr  = MPI_Win_lock( lock_type, irank, iassert,pred2_win);
                 assert( ierr == MPI_SUCCESS );
                 ierr = MPI_Win_lock( lock_type, irank, iassert,queue2_win);
                 assert( ierr == MPI_SUCCESS );
                 };
#endif
...
#ifdef USE_MPI_WIN_LOCK
              if (use_mpi_lock) {
                  ierr = MPI_Win_unlock( irank, queue2_win);
                  assert( ierr == MPI_SUCCESS );
                  ierr = MPI_Win_unlock( irank, pred2_win );
                  assert( ierr == MPI_SUCCESS );
                };
              };
#endif
